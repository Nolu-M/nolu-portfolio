...
const ContactSocialsLinks = ({ data, index}) => {
  ...
  return (
    <motion.a
        ...
        className={`w-12 h-12 rounded-full cursor-pointer ${theme === 'theme2' ? 'text-primary-green' : 'text-primary-pink'} relative p-[2px] cursor-pointer`}
    ...
    >
        <AnimatePresence>
            {isHoverred && (
                <motion.div
                    className={`absolute inset-1 blur-md ${theme === 'theme2' ? 'text-primary-green' : 'text-primary-pink'}`}>
                </motion.div>
            )}
